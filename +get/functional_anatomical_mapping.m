function[ypix_zplane] = functional_anatomical_mapping(ts,zs,tsync,opt)

arguments 
    ts struct % metadata from one of the tseries (tseries_md)
    zs struct % metadata from chosen z-stack (zstack_md)
...
% last updated 11-26-2025 CTP

% OUTPUTS
% ypix_zplane = 1 x nplanes cell array where each cell entry contains a
% 1 x ypix vector where every entry indicates the z plane that each row of
% y pixels should map to 
...
for p = 1:ts.nplanes
    ...
    ypix_zplane{p}= zvals; % gives z-distance in arbitrary z-units 
...
if opt.plot 
    ...
    for p = 1:ts.nplanes
        plot(max(zlocs)-ypix_zplane{p},'LineWidth',3)
