function [folder_tseries_frames]= folder_tseries_frames(ops)
%% DESCRIPTION
...
for i = 2:size(ops.filelist,1) 
    ...
    if curfolder == prevfolder
        ...
        folder_tseries_frames(count)=curframesum; % account for only 1 split per file, which will be overwritten 
    elseif curfolder>prevfolder || i== size(ops.filelist,1)    
        folder_tseries_frames(count)=curframesum; 
        count = count+1;
